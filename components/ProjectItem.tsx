...
export default function ProjectItem({
  ...
  return (
    <li className="grid grid-cols-7 max-lg:grid-cols-1 gap-16" ref={ref}>
      ...
      >
        <h3 className="text-3xl font-[500] text-violet-700">{title}</h3>
        <p className="text-xl leading-normal">{description}</p>
        <p className="text-violet-700 cursor-pointer hidden lg:block">
          {reverse ? (
        ...
        <p className="lg:hidden text-violet-700 cursor-pointer">
          Live demo &uarr;
        </p>
      </motion.div>
      <motion.a
        ...
        className={`project-image-container hover:before:hidden relative group rounded-2xl cursor-pointer border-2  border-gray-900 bg-gray-900 hover:border-violet-700 flip overflow-hidden ${
          reverse && "-order-1"
      ...
      >
        ...
        <BiLinkExternal className="absolute w-6 h-6 z-10 top-2 left-2 hidden group-hover:block text-violet-700" />
        <div className="absolute opacity-0 group-hover:opacity-100 transition-all duration-300 inset-0 bg-gradient-to-t from-violet-700/30 to-transparent pointer-events-none" />
      </motion.a>
    </li>
  );
}
